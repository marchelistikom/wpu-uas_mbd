...
if (isset($_POST['submit_pendaftaran'])) {
    ...
    move_uploaded_file($pas_foto_tmp, 'img_pendaftaran_au/' . $pas_foto_name);

    $skck_name = $_FILES['skck']['name'];
    $skck_tmp = $_FILES['skck']['tmp_name'];
    move_uploaded_file($skck_tmp, 'img_pendaftaran_au/' . $skck_name);
    ...
    if (isset($_FILES['sertifikat']) && $_FILES['sertifikat']['error'] == UPLOAD_ERR_OK) {
        ...
        move_uploaded_file($sertifikat_tmp, 'img_pendaftaran_au/' . $sertifikat_name);
    }

    // --- SINTAKS INPUT KE DATABASE ---
    $query_input = "INSERT INTO pendaftaran_au (
    ...
    if ($input) {
        echo '<script language="Javascript">
                ...
                document.location = "lihat_pendaftar_au.php";
              </script>';
    } else {
        echo '<script language="Javascript">
